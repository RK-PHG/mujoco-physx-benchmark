...
void setupWorld() {
    ...
    Eigen::VectorXd genVelocity(294);
    genVelocity << 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0,
            0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0,
            0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0,
            0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0,
            0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0,
            0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0, 0, 5, 25, 0, 0, 0,
    ...
    if(benchmark::bouncing::options.gui) {
        ...
        sim->cameraFollowObject(sim->getSingleBodyHandle(20), {10, 0, 10});
    }
}
...
//                                           benchmark::DART);
    ...
    std::ofstream outFile("mujoco_free_drop_test_result.csv");
    if (!outFile.is_open()) {
        std::cerr << "无法创建文件: " << "mujoco_free_drop_test_result.csv" << std::endl;
    }
